...
public class EventServiceTest {
	...
	void testAddEvent() throws EventManagementException {
        Event event = new Event("P101","Prasanth", LocalDate.parse("2021-10-21"),"Chennai",3000.0);
        String actual = service.add(event);
        assertEquals("P101", actual);
    }
	@Test
	@DisplayName("should add the event")
	void testAddEvent2() throws EventManagementException {
        Event event = new Event("P102","Party", LocalDate.parse("2021-09-20"),"Banglore",5000.0);
        String actual = service.add(event);
        assertEquals("P101", actual);
    }
}
